...
- Improved v1 CLI usability for the main 
ew/get flow: 
ew-app and get-app now auto-create/register a missing target bundle (so fresh --bundle ops does not fail after the bundles reset), and 
ew-app no longer forces interactive metadata prompts by default.
- Added --bundle option aliases (while keeping legacy --plug) for app commands (
ew-app, get-app, drop-app).
- Fixed the 3plug Bash launcher for Linux/macOS shells to auto-detect and use env/bin/3plug (with Windows path fallbacks), so users can run ./3plug ... directly without manual environment activation.
- Updated README.md to prefer direct launcher usage (./3plug / .\\3plug.cmd) instead of manual env activation for normal command use.

## Internal CLI Guide Split

- Added `doc/cli_internal_guide.md` for developers/publishers/platform operators.
- Kept `README.md` public-facing with only the allowed public command surface (`new`, `get`, `drop` at bundle/app level).
- Documented internal scaffold/regeneration workflow separately to support testing before mass generation.
## CLI Command Logging + Bundle Safety Cleanup

- Identified accidental bundle folders under `bundles/` (`branding&seo`, `doc`, `env`) caused by unsafe fallback bundle discovery when `config/plugs.txt` was empty.
- Removed unsafe root-folder fallback from `get_registered_plugs()` (bundles now come only from `config/plugs.txt` or actual `bundles/` directories).
- Added local CLI activity log: `logs/cli-commands.log` (records command runs via `3plug` when a real command is invoked).
- Cleaned stray folders from `bundles/` and restored bundle root to only `.gitkeep`.
## Backend Implementation Pattern Locked (v1)

- Added `doc/backend_implementation_guide.md`.
- Locked backend pattern: doc folders hold metadata/schema/actions declarations, while most executable logic lives in backend `actions/`, `services/`, and `queries/` (not in heavy Django models).
- Added a reusable backend update checklist to verify this pattern during future backend changes.
## Backend Template + Install Bridge Upgrade

- Upgraded `new-app` scaffold to generate the full backend template layout:
  - `backend/models`, `migrations`, `api`, `registry`, `schemas`, `actions`, `services`, `queries`, `filters`, `adapters/frappe`
  - backend stubs (`apps.py`, `api/urls.py`, registry metadata, package `__init__` files)
- Updated Django install helper (`install_django_app`) to detect the app backend scaffold and write a lightweight Django wrapper bridge manifest:
  - `manifold/<app>_app/backend_bridge.json`
  - `manifold/<app>_app/backend_registry.py`
- Updated `install-app` command to detect and report 3plug backend scaffold presence during install.
- Verified with a real test app scaffold and cleaned the temporary bundle/app afterward.
## PLT Planning JSON (Using Existing Docs)

- Generated `doc/planning/plt_planning_v1.json` from the existing docs (primarily `doc/rnd/plugs/plt/plt1.md` plus mapping/fetch/action guides).
- File includes:
  - `PLT` bundle planning (`platform_core` app)
  - module/submodule seed list
  - seeded business transactions/docs
  - doc action vocabulary reference
  - PLT backend fetch shape subset and pre-dashboard execution plan
- Current parsed counts:
  - modules: `18`
  - submodules: `54`
  - business transactions (seed): `162`
  - seed docs: `324`
## PLT Frappe Mapping Review (Planning)

- Generated `doc/planning/plt_frappe_mapping_review.json` from:
  - `doc/planning/plt_planning_v1.json`
  - `doc/files/frappe apps/frappe_doctypes_inventory.json`
  - `doc/mapping/doctype_to_doc_mapping_final.json`
  - `doc/mapping/doc_gap_classification.json`
- Confirmed direct finalized `PLT` mappings in the earlier global mapping file are `0` (expected because PLT was not in the refined mapping scope).
- Added a `PLT` module-by-module candidate doctype review list and a split between likely `borrow/adapt` vs likely native `PLT` modules.
## PLT Account-Type Requirements Lock (Planning)

- Reviewed `doc/rnd/platform/account.md` against `PLT` planning and confirmed the key gap: `PLT` planning had IAM/tenant modules but did not yet explicitly lock the account-type-first model as a planning artifact.
- Added `doc/planning/plt_account_requirements_v1.json` to capture:
  - `Business` vs `Publisher / Contributor` account types
  - required account domains (identity, roles, licensing, security, billing/revenue, localization, etc.)
  - `PLT` module alignment for account domains
  - backend fetch shape requirements (must include `account_type` and `ui.dir`)
  - prioritized `PLT` submodules/docs for account-focused review
## PLT Planning Folder Organization

- Created `doc/planning/plt/` and moved all `PLT` planning JSONs into it for focused review/tracking:
  - `doc/planning/plt/plt_planning_v1.json`
  - `doc/planning/plt/plt_frappe_mapping_review.json`
  - `doc/planning/plt/plt_doctype_to_doc_mapping_seed.json`
  - `doc/planning/plt/plt_account_requirements_v1.json`
## PLT Generation Fix + Clean Regeneration

- Patched `plug/utils/text.py::to_snake_case()` to sanitize filesystem-unsafe characters (`/`, `\\`, punctuation, Unicode dashes) and normalize names safely.
- Regenerated real `PLT` structure for `bundles/plt/platform_core` using `doc/planning/plt/plt_planning_v1.json`.
- Set `platform_core` git remote `origin` to `https://github.com/Triotek-Ltd/PLT.git`.
- Updated app metadata repo URL in:
  - `bundles/plt/platform_core/app.json`
  - `bundles/plt/platform_core/config/3plug.app.json`
- Recreated all planned PLT modules and submodules successfully:
  - modules: `18/18`
  - submodules: `54/54`
- Confirmed sanitized module/submodule IDs are now clean (no slash-based nested paths, no leading dash artifacts).
## PLT Doc Seeding + Harmonic Borrow + Backend Fetch Lock

- Seeded `PLT` docs for `bundles/plt/platform_core` from `doc/planning/plt/plt_planning_v1.json`.
- Seeded doc count verified on filesystem: `324/324` (`doc.json` under submodule docs folders).
- Each seeded PLT doc now includes:
  - planning metadata (`business_transaction_id`, titles)
  - classification (`bundle/app/module/submodule`, codes)
  - full doc action vocabulary in `actions.json`
  - mapping status (`mapped_candidate` or `build_from_scratch`)
- Applied PLT Frappe borrow/adapt seed mappings into doc metadata and `harmonic` adapter assets.
- Created harmonic borrow registry:
  - `bundles/plt/platform_core/backend/adapters/harmonic/plt_frappe_borrow_registry.json`
- Cached unique Frappe doctype source files (`json`/`py`/`js` when present) for mapped PLT candidates under:
  - `bundles/plt/platform_core/backend/adapters/harmonic/frappe_doctypes_cache/`
- Locked concrete PLT backend fetch contract for shell/dashboard work:
  - `doc/planning/plt/plt_backend_fetch_shape_v1.json`
- Noted DB runtime direction in future state:
  - local/dev may use SQLite
  - cloud-native target should use a stronger DB runtime (PostgreSQL-class target)

## CLI Refactor Note (Install Flow)

- Observed CLI/install design mismatch:
  - `new-app` / `get-app` currently generate the Django wrapper app (`manifold/<app>_app`) and bridge files
  - `install-app` currently focuses on site/runtime registration and migration
- Desired future refactor (after PLT dashboards/backend fetch stabilize):
  - `get-app` -> fetch/clone app repo only
  - `install-app` -> create wrapper + bridge + site registration/migrations/runtime integration
  - `new-app` -> scaffold-only (developer/publisher path)

## PLT Dashboard Plan Lock

- Created `doc/planning/plt/plt_dashboard_plan_v1.json` based on:
  - `doc/rnd/platform/dashboards.md`
  - `doc/rnd/platform/account.md`
  - current PLT planning/mapping/fetch files
- Locked PLT-first dashboard execution sequence (before wider bundle dashboards):
  - `PLT-D1` Landing / Entry dashboards
  - `PLT-D2` Platform Account / Profile dashboards
  - `PLT-D3` Platform-Level / Admin dashboards
  - `PLT-D4` Publisher / Contributor dashboards (PLT-needed subset)
- Explicitly deferred:
  - Developer/Builder dashboards (full implementation)
  - MGT dashboards
  - OPS/SP dashboards
  - ADM dashboards
- Added review checkpoints for:
  - doc-first terminology
  - account type visibility
  - RTL/LTR support
  - PLT fetch contract alignment

## PLT-D1 Dashboard Implementation Start (Home + Launcher)

- Implemented PLT metadata-backed Django endpoint stubs in `manifold/platform_core_app`:
...
- Endpoints return `doc`-first payloads with a shared response envelope and include `ui.dir` / locale context.
- Added frontend PLT fetch and resolver utilities:
  - `src/utils/platformFetch.js`
  - `src/utils/platformResolver.js`
- Started PLT dashboards using the new resolver (D1 scope only):
  - `/home` now renders `PltEntryDashboard` (Portal landing, account type selector stub, quick access launcher tiles, notifications/search stubs)
  - `/launcher` now renders `PltLauncherDashboard` (module/submodule launcher using real PLT metadata)
- Dashboard work remains scoped to `PLT-D1` pending review before `PLT-D2` account/profile dashboards.

## PLT Workspace Design Spec (Look-First Phase)

- Created `doc/planning/plt/plt_workspace_design_v1.md` to lock the workspace-first design/layout system before deeper dashboard implementation.
- Scope covers all PLT dashboards planned for the current phase:
  - `PLT-D1` Landing / Entry
  - `PLT-D2` Platform Account / Profile
  - `PLT-D3` Platform-Level / Admin
  - `PLT-D4` Publisher / Contributor (PLT subset)
- Locked:
  - workspace layout model (header, utility row, body grid, sections)
  - grid/spacing rules
  - RTL/LTR requirements
  - reuse strategy for `src/components/workspace/*`
  - PLT-specific workspace component folder structure
  - dashboard-by-dashboard layout specs (look first, content later)
## PLT Dashboard RTL/LTR Shell Refactor

- Added shared UI direction context for real-time `ltr`/`rtl` switching across the authenticated shell (`src/contexts/UiDirectionContext.js`).
- Wired app shell layout in `src/pages/_app.js` to honor shared direction (`dir` on root wrapper + row reversal for shell layout).
- Updated `Navbar`, `Sidebar`, and sidebar list items to mirror key directional UI details (icon spacing, back arrow, sidebar toggle placement, logo alignment).
- Connected PLT workspace direction toggle to shared shell direction state so the workspace and shell switch together.
- Kept page files thin (no page-level hardcoded direction logic).
## PLT Dashboard Design Spec Set (D1-D4)

- Created per-dashboard design/layout specs under `doc/planning/plt/dashboards/` for all PLT dashboard phases (`D1` to `D4`) before visual implementation.
- Added `doc/planning/plt/dashboards/README.md` as the index and working entry point.
- Each dashboard spec now includes:
  - route and user scope
  - Frappe workspace analog (what to borrow in UX/workspace behavior)
  - web/template layout references (Salero + related dashboard patterns)
  - look-first layout structure and required sections
  - RTL/LTR notes
- This locks the workflow the user requested: design/layout planning first, then visual implementation.

## PLT Dashboard UI Workflow Lock (Schema + Component Matrix)

- Added `doc/planning/plt/plt_dashboard_ui_schema_v1.json` to define the canonical dashboard visual composition schema (layout templates, section schema, navigation contract).
- Added `doc/planning/plt/plt_dashboard_component_matrix_v1.md` to lock the shared workspace component set (cards, charts, tables, feeds, route hub, forms, state components).
- Added `doc/planning/plt/plt_dashboard_visual_workflow_v1.md` to enforce the visual-first build process per dashboard before logic work.
- This resets dashboard implementation to a systematic process: spec -> schema -> shared components -> navigation pass -> RTL/LTR pass -> visual QA.

## PLT Screen Mapping & Dashboard-at-a-Time Workflow

- Added `doc/planning/plt/plt_dashboard_screen_map_v1.json` to define screens and transitions per dashboard (`D1`-`D4`), so each dashboard is treated as multiple process screens where needed.
- Added `doc/planning/plt/plt_dashboard_layout_registry_v1.md` to map every dashboard/screen set to an approved layout template (`hero_8_4_6_6`, `header_7_5`, `header_12_detail`, etc.).
- Added `doc/planning/plt/plt_dashboard_build_workflow_dashboard_at_a_time_v1.md` to enforce building one complete dashboard (all screens + navigation + RTL/LTR + visual QA) before moving to the next.
- This locks the user-requested process: dashboard-first, screen-aware, non-blind implementation.

## PLT Dashboard Planning Reorg (Per-Dashboard Folders)

- Reorganized `doc/planning/plt/dashboards/` into one subfolder per dashboard (`home_entry`, `launcher`, `account_*`, `admin_*`, `publisher_*`).
- Moved dashboard spec `.md` files into their respective dashboard folders.
- Added `dashboard.meta.json` inside each dashboard folder (generated from `plt_dashboard_screen_map_v1.json`) so every dashboard folder contains both markdown spec(s) and JSON planning metadata.
- Updated `doc/planning/plt/dashboards/README.md` to the new folder-based index and explicitly locked the “lean and on-point, not busy” visual rule.
