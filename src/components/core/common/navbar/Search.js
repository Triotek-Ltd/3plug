...
const Search = () => {
  ...
  return (
    <div className="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft w-80 xl:w-96">
      <span
        className={`text-xs md:text-sm ease-soft leading-5.6 absolute z-50 flex h-full items-center whitespace-nowrap border border-transparent bg-transparent py-1 px-2.5 text-center font-normal text-slate-500 transition-all ${
          isRtl
            ? "right-0 rounded-lg rounded-tl-none rounded-bl-none border-l-0"
            : "left-0 -ml-px rounded-lg rounded-tr-none rounded-br-none border-r-0"
        }`}
      >
        {" "}
      ...
      <input
        type="text"
        dir={dir}
        className={`text-sm focus:shadow-soft-primary-outline ease-soft w-full leading-5.6 relative block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-1 text-gray-700 transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none focus:transition-shadow ${
          isRtl
            ? "pr-8.75 pl-3 text-right"
            : "pl-8.75 pr-3 text-left -ml-px"
        }`}
      ...
      {isFocused && searchTerm && filteredOptions.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-1 z-10 max-h-60 overflow-y-auto rounded-lg border bg-white shadow-lg space-y-2">
          {filteredOptions?.map((option, index) => (
            <div
              key={index}
              className={`px-2 py-1 text-xs cursor-pointer hover:bg-gray-200 text-gray-700 ${isRtl ? "text-right" : "text-left"}`}
